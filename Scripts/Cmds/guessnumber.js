...
module.exports = {
  ...
  onStart: async function ({ message, event, getLang, commandName, args, globalData, usersData, role, api}) {
    ...
    api.sendPhoto(event.threadID, gameData.imageStream)
   .then(info => {
      global.GoatBot.onReply.set(String(info.message_id), {
        commandName,
  ...
  onReply: async ({ message, Reply, event, getLang, commandName, globalData, api }) => {
    ...
    if (gameData.isWin == null) {
      ...
      message.unsend(oldGameData.messageData.message_id);
      message.unsend(Reply.messageID);
      api.sendPhoto(event.threadID, gameData.imageStream, { caption: messageText });
...
function guessNumberGame(options) {
  ...
  const imageStream = canvas.toBuffer();
