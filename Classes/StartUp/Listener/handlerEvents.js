...
function getRole(threadData, senderID) {
  const adminBot = global.YukiBot.config.adminBot || [];
  if (!senderID)
    return 0;
...
function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang) {
  const config = global.YukiBot.config;
...
module.exports = function (api, threadModel, userModel, globalModel, threadsData, usersData, globalData) {
  return async function (event, message) {
    ...
    const { utils, client, YukiBot } = global;
    const { getPrefix, removeHomeDir, log, getTime } = utils;
    const { config, configCommands: { envGlobal, envCommands, envEvents } } = YukiBot;
    ...
    if (!threadData && !isNaN(threadID)) {
      ...
      if (
        ...
        && !global.db.receivedTheFirstMessage[threadID] && isGroup) {
        global.db.receivedTheFirstMessage[threadID] = true;
        const adminIDs = await threadsData.getadminIDs(threadID);
    ...
    async function onStart() {
      ...
      for (const cmdName in aliasesData) {
        if (aliasesData[cmdName].includes(commandName)) {
          command = YukiBot.commands.get(cmdName);
          break;
        }
      }
      // ————————————— SET COMMAND NAME ————————————— //
      ...
      try {
        ...
        const getText2 = createGetText2(langCode, `${process.cwd()}/Classes/Langs/Cmds/${langCode}.js`, prefix, command);
    ...
    */
    async function onChat() {
      const allOnChat = YukiBot.onChat || [];
      const args = body ? body.split(/ +/) : [];
      for (const key of allOnChat) {
        const command = YukiBot.commands.get(key);
        if (!command)
          continue;
        const commandName = command.config.name;
    ...
    */
    async function onAnyEvent() {
      const allOnAnyEvent = YukiBot.onAnyEvent || [];
      let args = [];
      ...
      for (const key of allOnAnyEvent) {
        ...
        const getText2 = createGetText2(langCode, `${process.cwd()}/Classes/Langs/Events/${langCode}.js`, prefix, command);
    ...
    */
    async function onFirstChat() {
      const allOnFirstChat = YukiBot.onFirstChat || [];
      const args = body ? body.split(/ +/) : [];
      ...
      for (const itemOnFirstChat of allOnFirstChat) {
        ...
        if (threadIDsChattedFirstTime.includes(threadID))
          continue;
        const command = YukiBot.commands.get(commandName);
        if (!command)
          continue;
    ...
    */
    async function onCallbackQuery() {
        const { onCallbackQuery } = YukiBot;
        const { messageID } = event; 
        const x = onCallbackQuery.get(messageID);
        if (!x) 
            return;

        const commandName = x.commandName;
        if (!commandName) {
            message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "cannotFindCommandName"));
            return log.err("onCallbackQuery", `Can't find command name to execute this callback!`, x);
        }

        const command = YukiBot.commands.get(commandName);
        if (!command) {
            message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "cannotFindCommand", commandName));
            return log.err("onCallbackQuery", `Command "${commandName}" not found`, x);
        }

        // —————————————— CHECK PERMISSION —————————————— //
        const roleConfig = getRoleConfig(utils, command, isGroup, threadData, commandName);
        const needRole = roleConfig.onCallbackQuery;
        if (needRole > role) {
            if (!hideNotiMessage.needRoleToUseCmdOnCallbackQuery) {
                if (needRole == 1)
                    return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminToUseOnCallbackQuery", commandName));
                else if (needRole == 2)
                    return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminBot2ToUseOnCallbackQuery", commandName));
            }
            else {
                return true;
            }
        ...
        try {
            ...
            log.info("onCallbackQuery", `${commandName} | ${userData.name} | ${senderID} | ${threadID} | ${event.data}`);
        } catch (err) {
            log.err("onCallbackQuery", `An error occurred when calling the command onCallbackQuery ${commandName}`, err);
    ...
    */
    async function handlerEvent() {
      const allEventCommand = YukiBot.eventCommands.entries();
      for (const [key] of allEventCommand) {
        const getEvent = YukiBot.eventCommands.get(key);
        if (!getEvent)
          continue;
        const commandName = getEvent.config.name;
        const getText2 = createGetText2(langCode, `${process.cwd()}/Classes/Langs/Events/${langCode}.js`, prefix, getEvent);
        const time = getTime("DD/MM/YYYY HH:mm:ss");
        try {
          ...
          if (typeof handler == "function") {
            await handler();
            log.info("EVENT COMMAND", `Event: ${commandName} | ${senderID} | ${userData.name} | ${threadID}`);
          }
        }
        catch (err) {
    ...
    */
    async function onEvent() {
      const allOnEvent = YukiBot.onEvent || [];
      const args = [];
        ...
        command.onEvent({
          ...
          .then(async (handler) => {
            if (typeof handler == "function") {
              try {
                await handler();
                log.info("onEvent", `${commandName} | ${senderID} | ${userData.name} | ${threadID}`);
              }
              catch (err) {
