...
module.exports = {
  ...
  onStart: async ({ args, message, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, commandName, event, getLang }) => {
    ...
    if (args[0] == "load" && args.length == 2) {
      ...
      const infoLoad = loadScripts("Events", args[1], log, configCommands, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang);
      ...
      for (const fileName of allFile) {
        const infoLoad = loadScripts("Events", fileName, log, configCommands, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang);
        infoLoad.status == "success" ?
      ...
      if (fs.existsSync(path.join(__dirname, "..", "Events", fileName)))
        return message.reply(getLang("alreadExist"), (info) => {
        ...
        const infoLoad = loadScripts("Events", fileName, log, configCommands, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang, rawCode);
        infoLoad.status == "success" ?
  ...
  onCallbackQuery: async function ({ Query, message, event, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang }) {
    const { author, messageID, data: { fileName, rawCode } } = Query;
    if (event.data != "confirm") return;
