...
const { isHexColor, colors } = require("./Classes/Console/colors.js");
const Prism = require("./Classes/Console/prism.js");
...
function formatNumber(number) {
  const regionCode = global.YukiBot.config.language;
  if (isNaN(number))
...
function messageFunc(bot, event) {
  const chatId = event.threadID;
  const messageId = event.messageID;

  return {
    send: (form, callback, options = {}) => {
      if (typeof form === 'string') {
        return bot.sendMessage(chatId, form, options)
          .then(info => {
            const result = {
              messageID: String(info.message_id),
              threadID: chatId,
              body: form,
              attachment: [],
              timestamps: info.date * 1000
            };
        ...
        if (form.body) {
          promises.push(
            bot.sendMessage(chatId, form.body)
              .then(info => {
                result.messageID = String(info.message_id);
                result.timestamps = info.date * 1000;
              })
          );
        }
        ...
        if (form.attachment) {
          promises.push(
            bot.sendPhoto(chatId, form.attachment)
              .then(info => {
                result.attachment = form.attachment;
                result.messageID = String(info.message_id);
              })
          );
        }
        ...
        if (form.sticker) {
          promises.push(
            bot.sendSticker(chatId, form.sticker)
              .then(info => {
                result.messageID = String(info.message_id);
              })
          );
        ...
        if (form.location) {
          promises.push(
            bot.sendLocation(chatId, form.location.x, form.location.z)
              .then(info => {
                result.messageID = String(info.message_id);
              })
          );
    ...
    reply: (form, callback, options = {}) => {
      const replyOptions = { reply_to_message_id: messageId };
      const OBJ = Object.assign({}, replyOptions, options);
      const promises = [];
      const result = {
        messageID: null,
        threadID: chatId,
      ...
      if (typeof form === 'string') {
        return bot.sendMessage(chatId, form, OBJ)
          .then(info => {
            result.messageID = String(info.message_id);
            result.timestamps = info.date * 1000;
        ...
        if (form.body) {
          promises.push(
            bot.sendMessage(chatId, form.body, replyOptions)
              .then(info => {
                result.messageID = String(info.message_id);
                result.timestamps = info.date * 1000;
              })
        ...
        if (form.attachment) {
          promises.push(
            bot.sendPhoto(chatId, form.attachment, replyOptions)
              .then(info => {
                result.attachment = form.attachment;
                result.messageID = String(info.message_id);
              })
        ...
        if (form.sticker) {
          promises.push(
            bot.sendSticker(chatId, form.sticker, replyOptions)
              .then(info => {
                result.messageID = String(info.message_id);
              })
          );
        ...
        if (form.location) {
          promises.push(
            bot.sendLocation(chatId, form.location.x, form.location.z, replyOptions)
              .then(info => {
                result.messageID = String(info.message_id);
              })
          );
...
function getMentionId(x) {
return global.db.allUserData.find(u => u.vanity == x)?.userID || "NOT_IN_DB";
}
...
const utils = {
  ...
  getExtFromMimeType,
  getExtFromUrl,
  getText: require("./Classes/Langs/makeFuncGetLangs.js"),
  getTime,
  getType,
  isHexColor,
  isNumber,
  jsonStringifyColor,
  loading: require("./Classes/Console/loading.js"),
  log,
  logColor: require("./Classes/Console/logColor.js"),
  randomString,
  randomNumber,
  removeHomeDir,
  splitPage,
