...
const { YukiBot } = global;
const { configCommands } = YukiBot;
  ...
  for (const folderModules of folders) {
    ...
    for (const file of Files) {
      ...
      try {
            ...
            if (!packageAlready.includes(packageName)) {
              ...
              if (!existsSync(`${process.cwd()}/node_modules/${packageName}`)) {
                const wating = setInterval(() => {
                  ...
                  loading.info('PACKAGE', `Installing package ${colors.yellow(packageName)} for ${text} ${colors.yellow(file)}...`);
                  count++;
                }, 80);
                try {
        ...
        if (YukiBot[setMap].has(commandName))
          throw new Error(`${text} "${commandName}" already exists with file "${removeHomeDir(YukiBot[setMap].get(commandName).location || "")}"`);
        ...
        if (aliases) {
          ...
          for (const alias of aliases) {
            ...
            if (YukiBot.aliases.has(alias))
              throw new Error(`alias "${alias}" already exists in ${text} "${YukiBot.aliases.get(alias)}" with file "${removeHomeDir(YukiBot[setMap].get(YukiBot.aliases.get(alias))?.location || "")}"`);
            validAliases.push(alias);
          }
          for (const alias of validAliases)
            YukiBot.aliases.set(alias, commandName);
        }
        // ——————————————— CHECK ENV GLOBAL ——————————————— //
        if (envGlobal) {
        ...
        // ——————————————— CHECK RUN ANYTIME ——————————————— //
        if (onChat)
          YukiBot.onChat.push(commandName);
        // ——————————————— CHECK ONFIRSTCHAT ——————————————— //
        if (onFirstChat)
          YukiBot.onFirstChat.push({ commandName, threadIDsChattedFirstTime: [] });
        // ————————————————— CHECK ONEVENT ————————————————— //
        if (onEvent)
          YukiBot.onEvent.push(commandName);
        // ———————————————— CHECK ONANYEVENT ———————————————— //
        if (onAnyEvent)
          YukiBot.onAnyEvent.push(commandName);
        // —————————————— IMPORT TO GLOBALYuki —————————————— //
        YukiBot[setMap].set(commandName.toLowerCase(), command);
        commandLoadSuccess++;
        // ————————————————— COMPARE COMMAND (removed in open source) ————————————————— //

        global.YukiBot[folderModules == "Cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
