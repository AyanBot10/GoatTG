...
const { client } = global;
const { configCommands } = global.YukiBot;
...
const options = {
  ...
  onCallbackQuery: async function ({ Query, message, event, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang }) {
    ...
    const { author, data: { fileName, rawCode } } = Query;
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getLang, rawCode) {
